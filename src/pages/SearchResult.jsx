...
const WhatsAppLink = ({ name }) => (
  <a
    href={`https://wa.me/9779860031044?text=Hi! I'm interested in: ${encodeURIComponent(name)}`}
    target="_blank"
    rel="noreferrer"
...
const SearchResult = () => {
  ...
  const { searchProducts, fetchProducts, products } =
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-7xl mx-auto px-4 py-12">
        {/* Header */}
        <div className="mb-8">
          <h1 className="font-main text-3xl text-[#145faf] mb-2">
            Search Results
          </h1>
          {searchQuery && (
            <p className="font-sub text-gray-600">
              Results for:{" "}
              <span className="font-semibold text-[#D93A6A]">
                "{searchQuery}"
              </span>
            </p>
          )}
        ...
        {!loading && !error && results.length > 0 && (
          <div>
            <p className="font-sub text-gray-600 mb-6">
              Found{" "}
              <span className="font-semibold text-[#145faf]">
                {results.length}
              </span>{" "}
      ...
      {toast.show && (
        <div
          className={`fixed bottom-4 right-4 font-sub px-6 py-3 rounded-xl shadow-lg text-white animate-pulse ${
