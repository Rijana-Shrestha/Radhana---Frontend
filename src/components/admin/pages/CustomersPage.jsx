...
const CustomersPage = () => {
  ...
  const filteredCustomers = useMemo(() => {
    return customers.filter((c) => {
      ...
      // High value = spent > 5000 OR orders > 3
      const isHighValue =
        (c.totalSpent || 0) > 5000 || (c.totalOrders || 0) > 3;
  ...
  // Stats — now computed from real order data returned by backend
  const stats = {
    total: customers.length,
    highValue: customers.filter(
      (c) => (c.totalSpent || 0) > 5000 || (c.totalOrders || 0) > 3,
    ).length,
